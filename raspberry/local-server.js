...
// AI 분석 결과 수신
// main.py로부터 AI 서버의 분석 결과를 받아 프론트엔드로 전달하는 API
app.post("/api/command-result", (req, res) => {
  const resultData = req.body; // main.py가 보낸 JSON 데이터 (분석 결과)
  console.log("✅ AI 분석 결과 수신! 프론트엔드로 전달:", resultData);

  // 연결된 모든 프론트엔드 클라이언트에게 분석 결과 전송
  sseClients.forEach((client) => {
    // 프론트엔드가 약속한 형식에 맞춰 payload에 전체 데이터를 담아 전송
    client.write(`data: ${JSON.stringify({ type: "COMMAND_RESULT", payload: resultData })}\n\n`);
  });

  res.json({ success: true, message: "Result forwarded to frontend" });
});
